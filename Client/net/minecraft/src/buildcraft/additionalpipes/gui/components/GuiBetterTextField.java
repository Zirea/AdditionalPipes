...
public class GuiBetterTextField extends Gui {
	...
	public GuiBetterTextField(GuiScreen guiscreen, FontRenderer fontrenderer, int x, int y, int width, int height, String text) {
	...
	public void textboxKeyTyped(char c, int keycode) {
		...
		if (keycode == Keyboard.KEY_RETURN) {
			if (parentGuiScreen instanceof IGuiIndirectButtons && returnbutton != null && returnbutton.enabled && returnbutton.drawButton) {
				((IGuiIndirectButtons) parentGuiScreen).buttonPressed(returnbutton);
			}
		}
	...
	public void mouseClicked(int x, int y, int button) {
		...
		if (isEnabled && x >= xPos && x < xPos + width && y >= yPos && y < yPos + height)// on
																							// the
																							// box
		{
			setFocused(true);
